...
- A production interval has a goal when its production metric contains a positive goal value.
...
Replace the current Trim-Saw-specific production subgroup helper in `people_performance.py` with a goal-based helper. The helper will consume the final role and final interval's production metric, then contribute one subgroup rank to the existing `PersonRow.sort_key`.
...
The subgroup uses the goal value already present in the final production metric. A missing, zero, negative, or unusable goal is treated as no goal and sorts in the lower metered subgroup. Existing unavailable-location and non-production behavior remains unchanged.
