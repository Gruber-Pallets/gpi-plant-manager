...
- [x] Write PostgreSQL tests for approval transitions, first/second approval,
  transaction rollback, duplicate updates, old versus fresh imports, and replay.
- [x] Run tests and confirm failure because the outbox does not exist.
- [x] Add `approval_source_updated_at` to the leave mirror and request `write_date`
...
- [x] Run the store tests against disposable PostgreSQL and existing sync tests.
...
- [x] Write tests for bilingual full/partial-day content, recipient rules,
...
- [x] Confirm tests fail before adding the worker.
- [x] Implement `run_once()` and a one-minute warmer. Read exact employee
...
- [x] Document the recipient policy and kill switch plus attention handling.
- [x] Run focused tests and review the complete diff.
