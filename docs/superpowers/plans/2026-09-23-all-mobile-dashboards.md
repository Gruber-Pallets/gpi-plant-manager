...
- [x] Capture immutable pre-change templates/static for desktop/TV screenshot comparisons.
- [x] Add inactive-by-default phone assets; activate only on Performance pages with sub-navigation and never TV.
- [x] Reuse authorized subnav links for the dashboard selector; compact top navigation into a touch-sized Menu. Preserve the already shipped Recycling shell.
- [x] Generalize grid mobile guard only to explicitly opted-in New and Operator pages.
- [x] Verify menu, switching pages, ranges, and breakpoint transitions.
...
- [x] Match approved mockup: full operator name above year-to-date/last30-day averages, readable day counts, qualifying thresholds in accessible details.
- [x] Keep Repairs/Dismantlers and New work-center families, real eligibility messages, full dates, and monthly awards. Stack awards and remove empty fixed-height areas on phones only.
- [x] Use shared phone shell; preserve desktop and TV markup/presentation.
- [x] Browser-check 320/390/760px and baseline desktop/TV screenshots.
...
- [x] Adapt approved Recycling patterns: operator-first labels with muted work centers; Now at100% on a120% goal track; red below goal, green at/above; neutral missing goal. Historical targets labelled Goal.
- [x] Preserve operators/stint attribution, links, work-center/date selection, all production/progress/uptime/downtime/GOAT/ribbon metrics in usable cards/details.
- [x] New/Operator opt into grid guard with `data-production-mobile` on non-TV html.
- [x] Browser-check mobile, missing data, multi-operator data, and baseline desktop/TV screenshots.
...
- [x] Stack People timelines full width under names; preserve filters, live updates and warning/assignment actions.
- [x] Make general/Forklift ranking rows readable with explicit labels and preserve sorting, drill-down, dates, categories and all metrics.
- [x] Contain trophy cards, controls, award dialogs, long names and month/year pickers on phones.
- [x] Verify at320/390/760, all authorized actions and modal bounds; compare desktop rendering unchanged.

## Task 5 — Verification and delivery (coordinator)

- [x] Review each implementer's diff and resolve material findings.
- [x] Run all new browser/template tests and applicable existing suites with an isolated local PostgreSQL database; run lint and JS syntax checks.
- [x] Compare desktop and TV against the captured baseline and inspect phone screenshots for every view.
...
Validation before deployment: 7,374 tests passed and 40 skipped against disposable local PostgreSQL in UTC. Ruff and JavaScript syntax checks passed. Desktop/TV baseline comparisons passed; People allows at most one RGB level for Chromium border raster rounding. Live deployment verification remains the final delivery step.
