 ...
 *
 * Instead we PROBE first: fetch the current URL, and only reload when it comes
 * back OK. On any failure we leave the last good frame on screen and re-check
 * sooner, so a brief outage is invisible on the floor and the board snaps back
 * on its own once the backend is answering again.
 */
(function () {
  "use strict";
  ...
  function check() {
    fetch(window.location.href, {
      ...
      .then(function (resp) {
        // resp.ok => a real 2xx page, safe to reload into. A 5xx edge error,
