...
def _holiday_rotation_bundle(
    *,
    status: str = "closed",
    holiday_odoo_id: int = 42,
    prepared: bool = True,
...
def _rotations_client(monkeypatch, *, raise_server_exceptions: bool = True):
    ...
    monkeypatch.setattr(
        ...
        lambda day, *, cur: rotations.saturday_recruiting_store.get(day, cur=cur),
    )
    monkeypatch.setattr(
        rotations.staffing,
        "load_schedule_for_update",
        lambda day, *, cur: deepcopy(rotations.staffing.load_schedule(day)),
    )
...
def test_rebuild_persists_schedule_on_saturday(monkeypatch):
    ...
    monkeypatch.setattr(
        rotations.saturday_recruiting_store,
        "get",
        lambda _day, **_kwargs: saturday_bundle,
    )
    monkeypatch.setattr(
        staffing_route,
        "_configured_center_capacities",
...
def test_open_holiday_auto_center_toggle_updates_recruiting_and_keeps_json_key(
    monkeypatch,
):
    client, rotations = _rotations_client(monkeypatch)
...
def test_holiday_auto_uses_only_effective_volunteers_and_optional_locks(
    ...
    client, rotations = _rotations_client(monkeypatch)
    staffing_route = _stub_recommendation_inputs(monkeypatch)
    bundle = _holiday_rotation_bundle()
    captured = {}
    saved = []
    prior = staffing.Schedule(
        day=BLACK_FRIDAY,
...
@pytest.mark.parametrize(
    ...
    [
        None,
        _holiday_rotation_bundle(status="recruiting", prepared=False),
        _holiday_rotation_bundle(holiday_odoo_id=99),
    ],
)
def test_unrecruited_or_open_holiday_cannot_auto(monkeypatch, bundle):
    client, rotations = _rotations_client(monkeypatch)
    saved = []
    monkeypatch.setattr(
        optional_workday,
...
def test_holiday_reset_uses_optional_volunteer_defaults_not_weekday_defaults(
    monkeypatch,
):
    client, rotations = _rotations_client(monkeypatch)
    staffing_route = _stub_recommendation_inputs(monkeypatch)
...
@pytest.mark.parametrize("reset_to_defaults", [False, True])
def test_cancelled_holiday_wins_rebuild_race(monkeypatch, reset_to_defaults):
    client, rotations = _rotations_client(monkeypatch)
    saved = []
    monkeypatch.setattr(
        optional_workday,
    ...
    monkeypatch.setattr(
        rotations.saturday_recruiting_store,
        "lock_for_schedule_mutation",
        lambda _day, *, cur: _holiday_rotation_bundle(status="cancelled"),
        raising=False,
    )
    monkeypatch.setattr(
        rotations.staffing,
    ...
    monkeypatch.setattr(
        rotations.staffing,
        "load_schedule_for_update",
        lambda day, *, cur: staffing.Schedule(day=day),
    )
    monkeypatch.setattr(rotations.staffing, "load_roster", lambda: [])
    monkeypatch.setattr(
        rotations.staffing,
...
def test_optional_classification_failure_blocks_auto_center_mutation(monkeypatch):
    client, rotations = _rotations_client(monkeypatch)
    saved = []
    monkeypatch.setattr(
        optional_workday,
