...
def test_run_batch_isolates_claim_errors_and_counts_outcomes(monkeypatch):
    ...
    monkeypatch.setattr(worker.task_delivery, "claim_due", MagicMock(return_value=[CLAIM, other]))
    ...
    worker.task_delivery.claim_due.assert_called_once_with(
        now=NOW, worker_id="task-worker", limit=10
    )
