...
# ---- negative_cap derivation (Birthday Pay "go negative by one day") ----


def test_fetch_leave_types_derives_negative_cap(monkeypatch):
    """A type configured in Odoo to allow a negative balance (e.g. Birthday
    Pay set to "go negative by one day": allows_negative + max_allowed_negative
    = 1) must surface a positive `negative_cap` so the kiosk lets the request
    through past a zero balance. Types without the setting get 0."""
    odoo_client._leave_types_cache = None
    responses = {
        ("hr.leave.type", "search_read"): [
            {"id": 1, "name": "Birthday Pay", "request_unit": "day",
             "requires_allocation": True, "allows_negative": True,
             "max_allowed_negative": 1.0, "color": 3, "active": True},
            {"id": 2, "name": "Paid Time Off", "request_unit": "day",
             "requires_allocation": True, "allows_negative": False,
             "max_allowed_negative": 0.0, "color": 2, "active": True},
        ],
    }
    _stub_execute(monkeypatch, responses)
    by_name = {t["name"]: t for t in odoo_client.fetch_leave_types()}
    assert by_name["Birthday Pay"]["negative_cap"] == 1.0
    assert by_name["Paid Time Off"]["negative_cap"] == 0.0


def test_fetch_leave_types_negative_cap_zero_when_flag_off(monkeypatch):
    """allows_negative False ⇒ cap is 0 even if Odoo carries a stray
    max_allowed_negative amount. Guards against enabling overdraw on a type
    the admin never actually flagged."""
    odoo_client._leave_types_cache = None
    responses = {
        ("hr.leave.type", "search_read"): [
            {"id": 1, "name": "Sick", "request_unit": "day",
             "requires_allocation": True, "allows_negative": False,
             "max_allowed_negative": 5.0, "color": 1, "active": True},
        ],
    }
    _stub_execute(monkeypatch, responses)
    assert odoo_client.fetch_leave_types()[0]["negative_cap"] == 0.0


def test_fetch_leave_types_negative_cap_defaults_when_fields_absent(monkeypatch):
    """Rows lacking the negative-cap fields default to negative_cap 0 rather
    than KeyError, so the picker keeps working on any Odoo version."""
    odoo_client._leave_types_cache = None
    responses = {
        ("hr.leave.type", "search_read"): [
            {"id": 1, "name": "PTO", "request_unit": "day",
...
def test_fetch_leave_types_falls_back_when_negative_fields_unknown(monkeypatch):
    """If the Odoo instance doesn't expose the negative-cap fields,
    search_read raises a Fault on the extended field list. fetch_leave_types
    must retry without those fields rather than break the whole picker,
    leaving negative_cap at 0."""
    odoo_client._leave_types_cache = None
    field_lists = []
    ...
    def fake(model, method, *args, **kwargs):
        ...
        if "allows_negative" in fields:
            raise xmlrpc.client.Fault(
                2, "Invalid field 'allows_negative' on model 'hr.leave.type'")
        return [{"id": 1, "name": "PTO", "request_unit": "day",
                 "requires_allocation": "yes", "color": 1, "active": True}]

    monkeypatch.setattr(odoo_client, "execute", fake)
    types = odoo_client.fetch_leave_types()
    assert types[0]["negative_cap"] == 0.0
    # First attempt carried the negative-cap fields (and faulted); the retry
    # dropped them.
    assert any("allows_negative" in fl for fl in field_lists)
    assert any("allows_negative" not in fl for fl in field_lists)
