...
def test_snapshot_for_range_uses_one_locked_generation_for_rows_open_ids_and_health(
    monkeypatch,
):
    now = at(4)
    ...
    health = attendance_mirror.MirrorHealth(
        last_incremental_completed_at=completed,
        last_full_sweep_completed_at=now - timedelta(hours=3),
        baseline_completed_at=at(),
        oldest_recalc_requested_at=None,
        last_error="poll failed",
        last_incremental_observed_at=observed,
    )
    raw = {
        **row(check_out=None),
        "last_seen_at": now - timedelta(minutes=3),
    }
    locked = attendance_mirror.AttendanceMirrorSnapshot(health=health, rows=(raw,))
    expected_span = attendance_timeline.LocationSpan(
        employee_odoo_id=41,
        ...
        app_work_center_name="Repair 1",
        odoo_work_center_id=71,
        odoo_work_center_name="Odoo Repair",
        attendance_ids=(101,),
    ...
    monkeypatch.setattr(
        ...
        lambda start, end: calls.append((start, end)) or locked,
    )
    monkeypatch.setattr(
        attendance_timeline.attendance_mirror,
        "health_snapshot",
        lambda: pytest.fail("separate health read is forbidden"),
    )
    monkeypatch.setattr(
        attendance_timeline.attendance_mirror,
        "rows_overlapping",
        lambda *_args: pytest.fail("separate row read is forbidden"),
    )
    monkeypatch.setattr(
        attendance_timeline,
        "_rows_with_employee_department_fallback",
        lambda rows, include_wage_type=False: tuple(rows),
    )
    monkeypatch.setattr(
        attendance_timeline,
        "_department_requirements_for_rows",
        lambda rows: lambda _department: True,
    )
    monkeypatch.setattr(
        attendance_timeline,
        "project_rows",
        lambda rows, **kwargs: (expected_span,),
    )

    snapshot = attendance_timeline.snapshot_for_range(
        at(), now, as_of_utc=now, map_work_center=lambda _wc: "Repair 1"
    )

    assert len(calls) == 1
    assert snapshot.spans == (replace(expected_span, is_open=False),)
    assert snapshot.open_employee_ids == frozenset()
    assert snapshot.verified_through_utc == completed
    assert snapshot.freshness_blockers == (
        "mirror_stale",
        "mirror_sync_failed",
...
def test_snapshot_open_ids_require_confirmation_from_latest_observation(monkeypatch):
    now = at(4)
    health = attendance_mirror.MirrorHealth(
        ...
        oldest_recalc_requested_at=None,
        last_error=None,
        last_incremental_observed_at=now - timedelta(seconds=1),
    )
    raw = {
        **row(check_out=None),
        "last_seen_at": now,
    ...
    monkeypatch.setattr(
        attendance_timeline.attendance_mirror,
        "snapshot_overlapping",
        lambda *_args: attendance_mirror.AttendanceMirrorSnapshot(health, (raw,)),
    )
    monkeypatch.setattr(
        attendance_timeline,
        "_rows_with_employee_department_fallback",
    ...
    assert snapshot.open_employee_ids == frozenset({41})
    assert snapshot.spans[-1].is_open is True


def test_snapshot_freshness_marks_an_incomplete_baseline():
    now = at(4)
    health = attendance_mirror.MirrorHealth(
        last_incremental_completed_at=now,
        last_full_sweep_completed_at=now,
        ...
        oldest_recalc_requested_at=None,
        last_error=None,
        last_incremental_observed_at=now,
    )

    assert attendance_timeline._snapshot_freshness_blockers(
        attendance_mirror.AttendanceMirrorSnapshot(health, ()),
        as_of_utc=now,
    ) == ("baseline_incomplete",)


def test_snapshot_for_range_requires_verified_freshness_even_when_rows_are_empty(
    monkeypatch,
):
    health = attendance_mirror.MirrorHealth(None, None, None, None, None)
    monkeypatch.setattr(
        attendance_timeline.attendance_mirror,
        "snapshot_overlapping",
        lambda *_args: attendance_mirror.AttendanceMirrorSnapshot(health, ()),
    )

    with pytest.raises(RuntimeError, match="no verified freshness"):
        attendance_timeline.snapshot_for_range(at(), at(1), as_of_utc=at(1))
