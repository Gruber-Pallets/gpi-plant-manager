        ...
        lambda oid: [
            {
                ...
                "leave_date_to": date(2026, 7, 3),
            },
            {
                "id": 2,
                "kind": "time_off_denied",
                "leave_date_from": date(2026, 7, 1),
...
def test_holiday_cancellation_notification_is_named_and_spanish_first(monkeypatch):
    monkeypatch.setattr(timeclock, "_person_by_id", lambda pid: PERSON_ES)
    monkeypatch.setattr(
        employee_notifications,
        "list_unacknowledged",
        lambda oid: [
            {
                "id": 1,
                "kind": "saturday_work_cancelled",
                "title": "Holiday work cancelled",
                "body": "Black Friday work was cancelled. Do not report to work.",
            }
        ],
    )
    ...
    assert response.status_code == 200
    assert response.text.index("Trabajo del día festivo cancelado") < response.text.index(
        "Holiday work cancelled"
    )
    assert "El trabajo de Black Friday fue cancelado. No te presentes a trabajar." in (
        response.text
    )
    assert "Black Friday work was cancelled. Do not report to work." in response.text
...
def test_notifications_screen_rejects_bad_token():
    resp = client.get("/timeclock/notifications/not-a-real-token", follow_redirects=False)
    assert resp.status_code == 303
...
def test_clock_out_holiday_reminder_is_named_and_spanish_first(monkeypatch):
    from datetime import datetime, timezone
    from zira_dashboard import (
        auto_lunch,
        saturday_work_reminder,
    ...
    monkeypatch.setattr(
        timeclock,
        "_open_log_row",
        lambda *a, **k: (1, datetime(2026, 11, 25, 22, 0, tzinfo=timezone.utc)),
    )
    monkeypatch.setattr(auto_lunch, "note_employee_clock_out", lambda oid: None)
    monkeypatch.setattr(timeclock_sync, "sync_one_by_id", lambda lid: None)
    monkeypatch.setattr(time_off_reminder, "reminder_for_person", lambda oid, today: None)
    monkeypatch.setattr(
        ...
        lambda person_id, today, now: {
            ...
            "hours": "7:00 AM–11:30 AM",
            "work_center": None,
            "title_key": "Holiday work reminder",
    ...
    assert response.status_code == 200
    assert response.text.index("Recordatorio de trabajo en día festivo") < (
        response.text.index("Holiday work reminder")
    )
    assert "Black Friday" in response.text
    assert "Friday, November 27" in response.text
