...
- **Go straight to a problem.** Schedule warnings now have buttons that take you to the right work center or person. Repeated warnings are grouped together.
- **Know when your work is saved.** The schedule says when changes are saving, saved as a draft, or posted. If saving fails, use Retry saving.
- **Use the schedule on your phone.** Work centers now appear as cards. You can switch back to the full table anytime.

#### Fixes

- **Keep changes when moving on.** Changing days or publishing waits for your schedule to finish saving.
- **Avoid repeated checks.** When the same schedule check is still running, the page waits for its answer. This cuts extra work on slow connections.
...
- **Only people you invite can sign in with their company account.** Admins can add people, choose their access, and remove access in Users & Access.
- **Choose from four roles.** Admins control the app. HR can work with private employee details. Managers can run daily work without seeing pay or private HR details. Visitors can only view dashboards.
- **Access changes apply on the next request.** Someone who is already signed in loses access when you remove it.
- **Shop-floor TVs keep working as before.**
...
- GOAT Watch now names the people who made pallets at that station, even after they leave. When people share the work, it shows all their names.

## 2026-09-11

### One inbox note per machine, not one per meter tick
...
- **Old finished notes now clear themselves.** Two kinds of Inbox notes — Saturday Recruiting and Unexpected Workers — could show up but could never tick themselves off once the problem went away. Nine of them had been stuck in the background since July. They clear on their own now, and a new safety check makes sure no future kind of note can get stuck the same way.
...
- **The step-by-step build plan is ready.** It includes safety checks for the Plant Scheduler and midday bars. Nothing on screen has changed yet.
...
- **The What’s new dot will clear only after that tab loads.** Opening Send feedback or My feedback will leave it alone.
...
### Plan anniversary PTO reminders for employees

- **A design is ready to remind employees when a work anniversary is close and they still have PTO.** Employees will confirm the reminder before punching in. Managers will be able to see that history on each employee's Staffing page. Nothing in the app has changed yet.
...
- **Breakdown time now stays correct after a late update or connection problem.** Workers with the same name stay separate, their alerts remain safe when live location data cannot be trusted, and dismissing or undoing a breakdown will not leave extra time behind.
- **Plant Manager will not move a worker from an old or closed breakdown alert.** It checks the worker and the alert first, so a rejected move does not change their time.
...
### Pay a past absence with PTO plan

- **A plan is ready to let a worker ask to use PTO for a missed day.** The missed day will still count as an absence, and a manager must approve the pay change. Nothing in the app has changed yet.
...
- **The time clock now keeps private special-day messages separate from important work updates.** This helps each worker see a birthday or work-anniversary message just for them before they clock in.
...
- **A plan is ready to put the light-bulb updates button at the top right of the time clock.** It will show the same updates and feedback screen used in Plant Manager. Nothing at the clock has changed yet.

### Birthday and work-anniversary build plan

#### Improvements

- **A step-by-step build plan is ready for private birthday and work-anniversary messages at the time clock.** It will help the app remember a special day until the worker comes back. Nothing new is showing at the clock yet.
...
- **The feedback safety plan now closes four small gaps before building starts.** Pictures, warnings, stopped work, and review actions each have one clear safe path.
...
- **Forklift on-time and late counts show up again.** The app now reads those flags from each finished forklift call, so the Performance boards can show how often drivers beat the clock.
...
- **The time clock now protects its list of names during updates.** Changing a person's Reserve setting cannot hide everyone. If no names can be shown, workers see a message to get a manager instead of a blank screen.
...
- **The app's automatic checkup now runs correctly.** This helps catch problems before an update goes out.
...
- **Saturday recruiting saves the schedule you can see.** Recruit now checks the current empty schedule again before it starts, so old hidden assignments do not get in the way.
