...
- "The defaults" = the clean-slate complete rebuild (`rotations.default_complete_schedule`): the goal-button engine run with no base assignments, no manual locks, and no prior sources. A brand-new future day is seeded with it on first view (mode `normal`, the default Auto centers; falls back to defaults-only placement when the solve is unavailable/incomplete), and Reset to defaults produces the same state: it discards every assignment — manual picks and non-Auto centers included — then runs the complete rebuild, returning 422 with placement issues and keeping the prior schedule when the rebuild can't place everyone (decided by Dale 2026-07-21; supersedes the 2026-07-14 reset-defaults-only and 2026-07-17 future-draft-defaults specs).
- The clean-slate rebuild (seed + reset) runs `minimum_only=False`: it fills past center minimums up to capacity, because it runs unattended and must place everyone even when available people exceed the enabled minimum slots or a center's exact defaults exceed its minimum (prod: Work Orders runs 3 exact defaults on a min-1 center — a minimum-capped solve deterministically strands 2 of them, which is what froze new days near-blank until 2026-07-23). Only the non-reset goal button staffs to minimum crew (`minimum_only=True`), paired with the Auto-selection advisory.
- The seed's defaults-only fallback is display-only and must NEVER be persisted: a saved row makes `schedule_revision` non-None, so every later view skips seeding and the day is frozen in the near-blank fallback (the 05:00-UTC page-warmer rows in prod). Only a complete engine solve may create the day's row.
